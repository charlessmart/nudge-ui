...
import { instrumentRenderedHtml } from "./identity.ts";

describe("instrumentRenderedHtml", () => {
...
<html>
  ...
  <body>
    ...
    const result = instrumentRenderedHtml(source, { projectRoot: "/work/site" });
  ...
  it("keeps relative annotation paths as authored without a project root", () => {
    ...
    const result = instrumentRenderedHtml(source);
...
<body>
  ...
  it("relativizes absolute annotation paths against the given project root per call", () => {
    ...
    const siteRoot = instrumentRenderedHtml(source, { projectRoot: "/work/site" });
    const workRoot = instrumentRenderedHtml(source, { projectRoot: "/work" });
    const noRoot = instrumentRenderedHtml(source);
