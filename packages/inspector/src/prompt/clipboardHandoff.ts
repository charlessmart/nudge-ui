 ...
 */
export async function reconcileClipboardHandoff(): Promise<number> {
  ...
  const removed = await verifyAndReconcileHandoff({ changes, structuralChanges });
  pruneCheckpoint();
  if (removed > 0) {
...
export function startClipboardHandoffController(doc: Document = document): () => void {
  ...
  const schedule = (): void => {
    ...
    timer = setTimeout(() => {
      ...
      if (stopped || checking || !checkpoint) return;
      checking = true;
      void reconcileClipboardHandoff()
        .catch(() => undefined)
        .finally(() => {
