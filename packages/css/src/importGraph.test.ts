...
const BROWSER_SAFE_ROOTS = ["src/index.ts", "src/model", "src/value-semantics"];
const NODE_PATH_ROOT = "src/token-inventory";
...
describe("@design-tool/css import graph (browser-safe vs Node/build-time)", () => {
  ...
  it("the browser-safe root entry never re-exports the token-inventory Node path", () => {
    const rootEntry = readFileSync(join(SRC, "src", "index.ts"), "utf8");
